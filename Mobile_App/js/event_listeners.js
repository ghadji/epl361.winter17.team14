ons.ready(function() {
    ...
    document.addEventListener("init", function(event) {
        ...
        //if initial page has no articles to show then switch to corresponding page
        if (page.matches("#ArticlesPage")) {
            if (!window.data.articlesToShow ||
                (window.data.articlesToShow && window.data.articlesToShow.length == 0)
            ) {
        ...
        //if current page is settings then get all toggles and values and store them
        if (page.matches("#settings")) {
            document.getElementById("NightModeToggle").checked = JSON.parse(
                window.localStorage.getItem("isNightMode")
            );
            document.getElementById("CacheNewsToggle").checked = JSON.parse(
                window.localStorage.getItem("isCachingNews")
            );
            document.getElementById("fontSizeText").innerHTML =
                JSON.parse(window.localStorage.getItem("fontSizeIndex")) * 2 +
                20 +
                "px";
            document.getElementById("MarkupToxicToggle").checked = JSON.parse(
                window.localStorage.getItem("isMarkupToxic")
            );
            document.getElementById("HideToxicToggle").checked = JSON.parse(
                window.localStorage.getItem("isHideToxic")
            );
            document.getElementById("ToxicToleranceSlider").value = JSON.parse(
                window.localStorage.getItem("toxicToleranceValue")
            );
        ...
        //build the list with all the articles
                ...
                articlesList.delegate = {
                    createItemContent: function(i) {
                        return ons.createElement(
                            '<ons-list-item tappable style="padding-left:1%; padding-right:1%;" onclick="fn.openArticle(' +
                            i +
                            ')">' +
                            '<ons-col width="30%">' +
                            '<img height="70%" width="100%" src="' +
                            window.data.articlesToShow[i].PictureSrc +
                            '">' +
                            "</ons-col>" +
                            "<ons-col>" +
                            '<div class="articleStyle" style="padding-left:5%;">' +
                            "<fontChangeTag>" +
                            window.data.articlesToShow[i].Title +
                            "</fontChangeTag>" +
                            "</div>" +
        ...
        //build the list with the sources
        if (page.matches("#addSources")) {
            ...
            sourcesList.delegate = {
                createItemContent: function(i) {
                    return ons.createElement(
                        '<ons-list-item tappable style="padding-left:1%; padding-right:1%;">' +
                        '<label class="right">' +
                        '<ons-checkbox id="source' +
                        i +
                        '" input-id="' +
                        window.data.sources[i].URL +
                        '"></ons-checkbox>' +
                        "</label>" +
                        '<label for="' +
                        window.data.sources[i].URL +
                        '" class="center menuStyle">' +
                        "<fontChangeTag>" +
                        window.data.sources[i].Category +
                        " - " +
                        window.data.sources[i].Title +
                        "</fontChangeTag>" +
                        "</label>" +
                        "</ons-list-item>"
                    );
        ...
        //fill the view elements with the article
    ...
    document.addEventListener("change", function(event) {
        ...
        if (markup && hide)
            if (markup.checked) hide.disabled = true;
            else if (hide.checked) markup.disabled = true;
        else if (!markup.checked && hide.disabled) hide.disabled = false;
        else if (!hide.checked && markup.disabled) markup.disabled = false;
    });

    //before showing modal for login check if user wanted to remembmer account
    document.addEventListener("preshow", function(event) {
        var modal = event.modal;

        if (event.modal) {
            var checkbox = document.getElementById("rememberme");
            if (window.localStorage.getItem("remember_account") == "true") checkbox.checked = true;
            else checkbox.checked = false;
            var username = document.getElementById("username");
            var password = document.getElementById("password");
            if (checkbox.checked) {
                username.value = window.localStorage.getItem("last_username");
                password.value = window.localStorage.getItem("last_password");
            } else {
                username.value = "";
                password.value = "";
            }
    ...
    //before hiding login modal check if user wants to store username and password
    document.addEventListener("prehide", function(event) {
        ...
        if (event.modal) {
            ...
            if (checkbox.checked = true) {
                var username = document.getElementById("username");
                var password = document.getElementById("password");
                window.localStorage.setItem("last_username", username.value);
                window.localStorage.setItem("last_password", password.value);
            } else {
                window.localStorage.setItem("last_username", "");
